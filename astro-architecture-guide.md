...
*This document is intended for publication on the ApostropheCMS docs site at `https://docs.apostrophecms.org/guide/astro-architecture.html`. It is the Part B companion to the in-repo `ARCHITECTURE.md` file.*

---

This guide explains the key patterns and conventions in the ApostropheCMS + Astro starter. It pairs with the in-repo `ARCHITECTURE.md` quick reference and is aimed at developers who are new to ApostropheCMS and want to understand how the two halves of the architecture fit together before extending the starter. It covers the bridge package, the component registry, area fields, link resolution, and image rendering — the patterns you will encounter in the first hour of working in the codebase.
...
import AposLayout from '@apostrophecms/apostrophe-astro/components/AposLayout.astro';

const aposData = await aposPageFetch(Astro);
if (aposData.redirect) {
  return Astro.redirect(aposData.url, aposData.status);
}
---
<AposLayout {aposData}>
  <!-- AposLayout selects the right template from the registry and renders it -->
</AposLayout>
```

`aposPageFetch` authenticates with the backend using `APOS_EXTERNAL_FRONT_KEY`, fetches the page document and all its widget data in a single request, and returns an `aposData` object. `AposLayout` then looks up the correct Astro component from the template registry and renders it, passing the content down as props.
...
**The key-matching rule is strict: the key must be the exact backend module name.** A mismatch produces no error — the component silently falls back to a default renderer, which can be very confusing to debug.

Here is what happens when you add a new `testimonial` widget:

**Before** (widget exists in the backend, but is not yet registered):
...
const widgetComponents = {
  ...
  'hero': HeroWidget,
  // 'testimonial' is missing — any testimonial widget on the page renders nothing useful
};
```

**After** (correctly registered):
...
const widgetComponents = {
  '@apostrophecms/rich-text': RichTextWidget,
  'hero': HeroWidget,
  'testimonial': TestimonialWidget, // key matches the backend module name exactly
};
```

The same rule applies in `frontend/src/templates/index.js` for page types. Scoped module names like `'@apostrophecms/rich-text'` and index/show variants like `'article-page:index'` must be written exactly as they appear in the backend.
...
export default {
  ...
  fields: {
    add: {
      main: {
        type: 'area',
        options: fullConfig  // which widgets editors can add here
      }
    }
  }
};
...
**The `_` prefix.** Any field whose name starts with `_` is a relationship field that Apostrophe resolves at request time. These always come back as arrays, even when the schema says `max: 1`. Use optional chaining and `[0]` consistently: `widget._image?.[0]`, `article._author?.[0]?.title`.

**`backend/lib/` utilities.** `lib/link.js` exports the canonical link field set; spread it into any schema that needs a link rather than copying the fields. `lib/area.js` exports three area configurations for different editorial contexts — import the right one rather than defining widget lists inline.
